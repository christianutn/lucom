...
export const getDetalleBafPorId = async (req, res, next) => {
    const { venta_id } = req.params;
    try {
        const detalleBaf = await Detalle_Baf.findOne({
            where: {
                venta_id: venta_id
            },
            include: [
        ...
        if (!detalleBaf) {
            return next(new AppError(`Detalle de BAF con ID ${id} no encontrado`, 404));
        }
        res.status(200).json(detalleBaf);
    } catch (error) {
        next(new AppError('Error al obtener el detalle de BAF', 500));
    }
};
