...
import {  getTipoNegocio, createTipoNegocio, getTipoNegocioPorId, actualizarTipoNegocio, eliminarTipoNegocio} from '../controllers/tipo_negocio.controller.js';
...
router.put('/:id',
    ...
    [
        param('id').exists().isInt({ min: 1 }).withMessage('El ID debe ser un número entero positivo')
            .custom(async (value) => {
                const tipo_negocio = await TipoNegocio.findByPk(value);
                if (!tipo_negocio) {
